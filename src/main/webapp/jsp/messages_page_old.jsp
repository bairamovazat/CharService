...
<div class="container-fluid">
    <div class="row">
        ...
        <div id="messages" class="col-9 col-sm-12">
            <h1>Сообщения</h1><br>
            <c:forEach items="${messages.keySet()}" var="companion">

            <c:if test="${activeMessages != null && activeMessages == companion.getId()}">
            <div id="${companion.getId()}" class="card collapse show" data-parent="#messages">
                </c:if>
                <c:if test="${activeMessages == null || activeMessages != companion.getId()}">
                <div id="${companion.getId()}" class="collapse" data-parent="#messages">
                    </c:if>
                    <div class="card-body">
                        <h5 class="card-title">${companion.getName()}</h5>
                        <c:forEach items="${messages.get(companion)}" var="message">
                            <small class="card-text">${message.getSendDate()} : ${message.getUser().getName()}</small>
                            <div class="card-text">
